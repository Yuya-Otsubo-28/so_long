...
char	*next_store(char *store)
{
	...
	next_store = NULL;
	head = ft_strclen(store, '\n') + 1;
	tail = ft_strclen(store, '\0');
	next_store = ft_substr(store, head, tail - head);
	free(store);
	return (next_store);
}
...
char	*make_res(char *store)
{
	...
	res = NULL;
	if (ft_strchr(store, '\n'))
		res_size = ft_strclen(store, '\n') + 1;
	else
		res_size = ft_strclen(store, '\0');
	res = ft_substr(store, 0, res_size);
	return (res);
}
