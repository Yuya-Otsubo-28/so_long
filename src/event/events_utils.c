...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 08:53:52 by yotsubo           #+#    #+#             */
/*   Updated: 2022/11/21 08:56:25 by yotsubo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	search_player(t_map *map, t_pt *p)
{
	...
	while ((size_t)p->y < map->num_of_line)
	{
		...
		{
			if (map->map_data[p->y][p->x] == 'P'
				|| map->map_data[p->y][p->x] == P_ON_C
				|| map->map_data[p->y][p->x] == P_ON_E)
				return ;
			p->x++;
		}
		p->y++;
