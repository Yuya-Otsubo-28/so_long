...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/21 09:12:38 by yotsubo           #+#    #+#             */
/*   Updated: 2022/11/21 09:21:46 by yotsubo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_wall(t_map *map)
{
	...
	while (i + 1 < map->num_of_line)
	{
		if (map->map_data[i][0] != '1'
			|| map->map_data[i][map->num_of_column - 1] != '1')
			map_errors(map->map_data, map->num_of_line, INPUT_ERROR);
		i++;
	}
...
static void	count_init(t_count *count)
{
	count->p_count = 0;
	count->c_count = 0;
	count->e_count = 0;
	...
	while (map->map_data[++i])
	{
		...
		while (map->map_data[i][++j] != '\n')
		{
			if (!(map->map_data[i][j] == '0' || map->map_data[i][j] == '1'
				|| map->map_data[i][j] == 'C' || map->map_data[i][j] == 'E'
				|| map->map_data[i][j] == 'P'))
				map_errors(map->map_data, map->num_of_line, INPUT_ERROR);
